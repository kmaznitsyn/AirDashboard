...
export default function WeatherCard({ w }: Props) {
  ...
  return (
    <View style={[styles.card, { backgroundColor: cardBg }]}>
      <Text style={styles.city}>{w.city}, {w.country}</Text>
...
function MetaPill({ icon, label, sublabel }: { icon: string; label: string; sublabel: string }) {
  return (
    <View style={styles.pill}>
      ...
      <Text style={styles.pillLabel}>{sublabel}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  city:      { fontSize: 20, fontWeight: '700', color: '#fff', textTransform: 'capitalize' },
  mainRow:   { flexDirection: 'row', alignItems: 'center', gap: 4 },
  icon:      { width: 64, height: 64 },
  temp:      { fontSize: 52, fontWeight: '800', color: '#fff', lineHeight: 60 },
  desc:      { fontSize: 15, color: 'rgba(255,255,255,0.8)', textTransform: 'capitalize', marginBottom: 4 },
  metaRow:   { flexDirection: 'row', gap: 8, marginTop: 4 },
  pill:      { flex: 1, backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 10, padding: 10, alignItems: 'center', gap: 2 },
  pillIcon:  { fontSize: 18 },
  pillValue: { fontSize: 14, fontWeight: '700', color: '#fff' },
